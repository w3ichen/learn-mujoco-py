...
cam.azimuth = 89.98786220715142
cam.elevation = -88.48674475955615
cam.distance = 5.096293371917445
...
mj.mj_forward(model, data)
# after step, which calculates the position of the end-effector, we can get the position of the end-effector
position_Q = data.site_xpos[0]
# print("position_Q", data.site_xpos, position_Q)
r = 0.5  # circle radius
...
while not glfw.window_should_close(window):
    ...
    while time - time_prev < 1.0 / 60.0:
        ...
        # Compute Inverse Jacabian Jinv
        Jinv = np.linalg.inv(J)

        # Compute dX
        X_ref = np.array([x_ref[i], y_ref[i]])  # Reference position
        X = np.array([position_Q[0], position_Q[1]])  # Measured position
        dX = X_ref - X
