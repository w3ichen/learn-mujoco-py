...
xml_path = "2D_double_pendulum_lqr.xml"  # xml file (assumes this is in the same folder as this file)
...
# Create the function xdor = f(x,u)
def f(x, u):
    data.qpos[0] = x[0]
    data.qpos[1] = x[1]
    data.qvel[0] = x[2]
    ...
    tau = np.array([u[0], 0])
    qdot = np.matmul(invM, np.subtract(tau, frc_bias))

    xdot = np.array([data.qvel[0], data.qvel[1], qdot[0], qdot[1]])
    return xdot
