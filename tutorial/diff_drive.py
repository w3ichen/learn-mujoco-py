...
def quat2euler(quat_mujoco):
    # mujoco quat is [const, x, y, z]
    # scipy quat is [x, y, z, const]
    quat_scipy = np.array(
...
def controller(model, data):
    ...
    # ctrl based on the order of actuator in the xml file
    data.ctrl[0] = 10
    data.ctrl[1] = 0
...
cam.azimuth = 90.0
cam.elevation = -45.0
cam.distance = 10.145554787016227
...
while not glfw.window_should_close(window):
    ...
    # [0, 1, 2] is the position
    # print(data.qpos[0])
    # print(data.qpos[1])
    # print(data.qpos[2])

    # [3, 4, 5, 6] is the quaternion rotation
    quat = np.array([data.qpos[3], data.qpos[4], data.qpos[5], data.qpos[6]])
    euler = quat2euler(quat)
    # print("yaw = ", euler[2])  # z-axis
